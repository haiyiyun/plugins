...
func (self *Service) Route_GET_List(rw http.ResponseWriter, r *http.Request) {
	...
	typ, _ := strconv.Atoi(typeStr)
	publishTypeStr := r.URL.Query().Get("publish_type")
	publishType, _ := strconv.Atoi(publishTypeStr)
	...
	valid.Digital(publishTypeStr).Key("publish_type_str").Message("publish_type必须数字")
	valid.Have(publishType,
		predefined.ContentPublishTypeDynamic,
		predefined.ContentPublishTypeArticle,
		predefined.ContentPublishTypeQuestion,
		predefined.ContentPublishTypeAnswer,
	).Key("publish_type").Message("publish_type必须是支持的类型")
	...
	if categoryIDStr != "" {
		valid.BsonObjectID(categoryIDStr).Key("category_id").Message("category_id必须支持的格式")
	}

	if subjectIDStr != "" {
		valid.BsonObjectID(subjectIDStr).Key("subject_id").Message("subject_id必须支持的格式")
	}
	...
	if categoryID != primitive.NilObjectID {
		filter = append(filter, contentModel.FilterByCategoryID(categoryID)...)
	}

	if subjectID != primitive.NilObjectID {
		filter = append(filter, contentModel.FilterBySubjectID(subjectID)...)
	}
